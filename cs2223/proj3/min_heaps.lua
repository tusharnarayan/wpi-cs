-- Heaps relative to the ordering <
-- (min-heaps)
--
...
local function make_empty_heap()
   return { heap_bound = 0 }
end
...
local function heapify_up(h,i)
   ...
   if p and h[i]<h[p]           -- child too small!
   then
      -- flip em!
      h[p], h[i] =
	 h[i], h[p]
...
local function heapify_down (h,p)
   ...
   if h.heap_bound < r		-- no right child at all?
      or h[l]<h[r]		-- or both exist but left is less
   then
      f = l			-- take l
   else
      f = r			-- otherwise use r
   ...
   if  h[f]<h[p]		-- if favored child belongs on top
   then                         -- then flip em!
      h[f], h[p] = h[p], h[f]
...
local function build_heap (a)
   local h =  make_empty_heap()
...
local function check_heap (h)
   if parent(h.heap_bound) then
      for p = 1,parent(h.heap_bound) do
         if h[left_child(p)]<h[p] -- troubled family!
