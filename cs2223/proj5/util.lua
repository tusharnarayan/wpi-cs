...
local function iterate_through_table(fn, tab)
   local result = {} 
...
local function iterate_through_two_tables(fn, tab1, tab2)
   local result = {} 
...
local function random_int_array (len, max_entry) 
   local result = {} 
   local ignore = math.random(max_entry) -- discard first value 

   for i = 1, len do 
      result[i] =
	 math.random(max_entry) -- take random number up to max_entry
   end 
   return result
end 
...
local function increasing_int_array(len) 
   local result = {} 
...
local function decreasing_int_array(len) 
   local result = {} 
...
local function assert_sorted (a,start,finish,s) 
   local last = a[start] 
...
local function array_ratios(a1,a2) 
...
util = 
   {
   iterate_through_table = iterate_through_table, 
   iterate_through_two_tables = iterate_through_two_tables, 
   identity = identity, 
   copy_table = copy_table, 
   fold = fold, 
   print_table = print_table, 
   print_array = print_array, 
   random_int_array = random_int_array, 
   increasing_int_array = increasing_int_array, 
   decreasing_int_array = decreasing_int_array, 
   invert = invert, 
   assert_sorted = assert_sorted, 
   derive = derive, 
   sum = sum, 
   write_dot = write_dot, 
   array_ratios = array_ratios, 
