...
int prettyPrintRestrainer2 = 4; //used to only print for 4 nodes when printing packets
int printCounter2 = 0; //counter for the number of nodes printed
...
void rtinit2() {
  ...
  neighbor2 = getNeighborCosts(myNode2);
  ...
  for(i = 0; i < MAX_NODES; i++){
    ...
    for(j = 0; j < MAX_NODES; j++){
      if(dt2.costs[i][j] < localMinimum){
	localMinimum = dt2.costs[i][j];
      }
    }
    min[i] = localMinimum;
  }
  ...
  if(TraceLevel >= 1){
    printdt2(myNode2, neighbor2, &dt2);
  }  
  
  //send an update packet to all the nodes in the network
  for(i = 0; i < neighbor2->NodesInNetwork; i++){
    if((i != myNode2) && (neighbor2->NodeCosts[i] != INFINITY)){
      distanceUpdatePacket.sourceid = myNode2; //source router is myNode2
	  ...
	  for(j = 0, printCounter2 = 0; (j < MAX_NODES) && (printCounter2 < prettyPrintRestrainer2); j++, printCounter2++){
		printf("%d   ", min[j]);
      }
      printf("\n");
	}
...
void rtupdate2( struct RoutePacket *rcvdpkt ) {
  ...
  if(TraceLevel >= 1){
	printf("At time t=%f, rtupdate2() called. node %d receives a packet from node %d\n", clocktime, myNode2, source);    
  }
  
  //update distance table
  for(i = 0; i < neighbor2->NodesInNetwork; i++){
    localMinCost = rcvdpkt->mincost[i];
    if(dt2.costs[i][source] > (dt2.costs[source][source] + localMinCost)){
      dt2.costs[i][source] = dt2.costs[source][source] + localMinCost;     
